...
async function loadProductProfitability() {
  try {
    ...
    if(tbody) tbody.innerHTML = '<tr><td colspan="12" style="text-align:center;">Loading raw data from database...</td></tr>';
...
function applyProfitFilters() {
  ...
  rawProfitProducts.forEach(p => {
    ...
    const gpUnit = currentSellingPrice - purchaseCost;
      ...
      if (String(oi.product_id) === String(p.id) || String(oi.product_name) === String(p.name)) {
        if (validOrderIds.has(String(oi.order_id))) {
          ...
          if (sDate >= fromTime && sDate <= toTime) {
            ...
            salesValue += val;
            const itemGp = val - (Number(oi.quantity || 0) * purchaseCost);
            actualGP += itemGp;
    ...
    const stockVal = currentStock > 0 ? (currentStock * purchaseCost) : 0;
    ...
    filteredProfitabilityData.push({
      ...
      purchaseCost, purchaseValue, currentSellingPrice, salesValue,
...
function calculateMoMPerformance() {
  ...
  const costMap = {};
  rawProfitProducts.forEach(p => { costMap[p.id] = Number(p.cost_price || 0); costMap[p.name] = Number(p.cost_price || 0); });
...
function renderProfitCharts() {
  ...
  const costMap = {}; rawProfitProducts.forEach(p => { costMap[p.id] = Number(p.cost_price || 0); costMap[p.name] = Number(p.cost_price || 0); });
...
function renderProfitabilityLists() {
  ...
  if(tLow) tLow.innerHTML = '<thead><tr><th>Product</th><th>Cost</th><th>Price</th><th>GP%</th></tr></thead><tbody>' + 
    renderTrs(lowMargin, d => \`<tr><td class="col-product-name" onclick="showProductProfitDetailModal('\${d.id}')">\${d.name}</td><td>₹\${d.purchaseCost.toFixed(0)}</td><td>₹\${d.currentSellingPrice.toFixed(0)}</td><td style="color:#b45309;">\${d.gpPercent.toFixed(1)}%</td></tr>\`) + '</tbody>';
  ...
  if(tLoss) tLoss.innerHTML = '<thead><tr><th>Product</th><th>Cost</th><th>Price</th><th>Loss</th></tr></thead><tbody>' + 
    renderTrs(lossMaking, d => \`<tr><td class="col-product-name" onclick="showProductProfitDetailModal('\${d.id}')">\${d.name}</td><td>₹\${d.purchaseCost.toFixed(0)}</td><td>₹\${d.currentSellingPrice.toFixed(0)}</td><td style="color:var(--danger);">₹\${d.actualGP.toFixed(0)}</td></tr>\`) + '</tbody>';
  ...
  // Inventory Aging logic (based on latest purchase for unsold stock)
  // We approximate aging by checking the latest purchase date of in-stock items
      ...
      <tr>
        ...
        <td style="text-align:right;">
          <div style="display:flex; align-items:center; justify-content:flex-end; gap:4px;">
            ...
            <input type="text" inputmode="decimal" class="price-edit-input" style="width:70px; border-color:var(--primary); font-weight:bold; color:var(--success); background:var(--surface);" value="\${d.currentSellingPrice.toFixed(2)}" onchange="updateProfitabilitySellingPrice('\${d.id}', '\${d.currentSellingPrice}', this.value)" />
          </div>
        </td>
...
async function updateProfitabilitySellingPrice(prodId, oldVal, newVal) {
  ...
  if (error) {
    ...
    if(idx !== -1) {
      ...
      }); // Approximate sync, bulk sync pushes exact
    }
    applyProfitFilters();
  }
}
...
function exportProfitabilityCSV() {
  ...
  let csv = "Product,SKU,Qty Purchased,Qty Sold,Current Stock,Purchase Cost,Current Selling Price,Sales Value,GP / Unit,Period GP%,Actual GP Generated,Potential GP\\n";
  filteredProfitabilityData.forEach(d => {
    csv += \`"\${d.name.replace(/"/g, '""')}","\${d.sku}",\${d.qtyPurchased},\${d.qtySold},\${d.currentStock},\${d.purchaseCost},\${d.currentSellingPrice},\${d.salesValue},\${d.gpUnit},\${d.gpPercent},\${d.actualGP},\${d.potentialGP}\\n\`;
  });
