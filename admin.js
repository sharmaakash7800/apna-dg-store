    ...
    if (isEditing) {
      return `
        <tr style="background: rgba(79, 70, 229, 0.12); border-left: 4px solid var(--primary);">
          ...
          <td><input type="number" min="0" id="edit_qty_${p.id}" class="price-edit-input" style="width:60px; text-align:center; border-color:var(--primary); background:var(--surface); color:var(--text-dark);" value="${qtyVal}" /></td>
          <td><div style="display:flex; align-items:center; gap:2px;"><span style="font-size:11px; color:var(--text-muted);">₹</span><input type="number" step="0.01" id="edit_cost_${p.id}" class="price-edit-input" style="width:70px; border-color:var(--primary); background:var(--surface); color:var(--blue);" value="${costPriceNum.toFixed(2)}" oninput="updateLiveUnitCost('${p.id}')" /></div></td>
          ...
          <td><div style="display:flex; align-items:center; gap:2px;"><span style="font-size:11px; color:var(--text-muted);">₹</span><input type="number" step="0.01" id="edit_selling_${p.id}" class="price-edit-input" style="width:70px; border-color:var(--primary); background:var(--surface); color:var(--success);" value="${sellingPriceVal}" /></div></td>
        ...
        <div class="item-row">
          ...
          <div class="dropdown-wrapper">
            ...
            <div class="dropdown-menu">
              ${s.phone ? `<a href="tel:${s.phone}" class="dropdown-item">📞 Call</a><a href="https://wa.me/${cleanPhone}?text=Hello%20${encodeURIComponent(s.name)},%20price%20confirm%20karna%20tha." target="_blank" class="dropdown-item">💬 WhatsApp Query</a>` : ''}
...
async function saveUpdatedPurchaseOrder() {
  ...
  const { error: poErr } = await db.from("purchase_orders").update({ supplier_name: supplierName, total_amount: totalAmount }).eq("id", poId);
